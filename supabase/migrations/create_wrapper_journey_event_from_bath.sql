-- =============================================
-- Migração: Wrapper 1 argumento + Backfill Maya
-- Arquivo: create_wrapper_journey_event_from_bath.sql
-- Objetivo:
...
BEGIN
  ...
  SELECT user_id INTO v_admin FROM public.admin_users_pet WHERE is_active = true LIMIT 1;
  IF v_admin IS NULL THEN
    RAISE EXCEPTION 'Nenhum admin ativo encontrado para fallback';
  END IF;

  -- Delegar para a função de 2 argumentos
  SELECT public.create_journey_event_from_bath(bath_id, v_admin) INTO new_event_id;
...
GRANT EXECUTE ON FUNCTION public.create_journey_event_from_bath(UUID) TO authenticated;
...
BEGIN
  ...
  LOOP
    BEGIN
      PERFORM public.create_journey_event_from_bath(r.bath_id);
