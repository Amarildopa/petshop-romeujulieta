-- =============================================
-- Correção de Integração: Criar eventos faltantes da Maya
-- Arquivo: corrigir-integracao-maya.sql
-- Objetivo: Para cada banho aprovado e marcado para jornada, sem evento,
--           criar o evento "Banho e Tosa" na Jornada da Maya.
-- =============================================
...
-- 2) Verificação pós-processo: banhos da Maya (mostrar journey_event_id)
WITH maya AS (
  SELECT p.id AS pet_id FROM public.pets_pet p WHERE p.name ILIKE 'Maya%'
)
SELECT 
  'BANHOS_APOS_CORRECAO' AS secao,
  wb.id,
  wb.pet_id,
  wb.pet_name,
  wb.bath_date,
...
-- 3) Verificação de eventos "Banho e Tosa" da Maya
WITH maya AS (
  SELECT p.id AS pet_id FROM public.pets_pet p WHERE p.name ILIKE 'Maya%'
)
SELECT 
